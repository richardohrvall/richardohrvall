...
Om man letar lite på nätet finns mängder av guider till LaTeX, så i det här blogginlägget tänkte jag bara tipsa om hur man kan använda Zotero för att få in referenser. Det gör man enklast med hjälp av nämnda tillägget till Zotero, BetterBibTeX. Det går att ladda ned [här](https://github.com/retorquere/zotero-better-bibtex). Därefter kan man i menyn Arkiv välja Exportera bibliotek för att få ut alla referenser som man har i Zotero till en så kallad bib-fil. En sådan filen används sedan för att lägga in referenser i LaTeX. Vid exporteringen går det att välja mellan "Better BibLaTeX" och "Better BibTeX", vilket påverkar utformningen av bib-filen. Vad man bör välja beror på vad man sedan använder i LaTeX (se t.ex. [här](https://tex.stackexchange.com/questions/25701/bibtex-vs-biber-and-biblatex-vs-natbib) för en beskrivning av skillnaderna). Om man bockar i rutan "Keep updated" så uppdateras bib-filen så fort en ny källa läggs till i Zotero.

Om man på så vis exporterar alla referenser från Zotero kan man hänvisa till den bib-filen i artiklar och andra dokument. Fast jag föredrar att skapa en specifik bib-fil för varje text. Fördelarna med det är dels att man kan lägga bib-filen i samma mapp som dokumentet, som man då kan dela med andra författare, dels att man efter att man är klar kan stänga av att filen ska uppdateras om något ändras i Zotero, så att man inte riskerar att någon referens tas bort av misstag.

Enklast gör man detta genom att skapa en "Ny samling" i Zotero, och dra dit alla referenser som man vill ha med i texten (varje referens kan finnas i fler än en samling, så det påverkar inte andra samlingar du redan har skapat). Sedan kan man högerklicka på den samlingen och välja "Exportera samling", och sedan på motsvarande sätt som tidigare välja BibTeX eller BibLaTex, och var filen ska läggas (förslagsvis i samma mapp som du har det dokument du skriver på). Om man dessutom bockar i rutan "Keep updated" kan man lägga in nya referenser i samlingen i Zotero och bib-filen uppdateras automatiskt. När man är säker på att man är färdig med dokumentet kan man koppla ifrån uppdateringen via Redigera/Inställningar/Better BibTeX och under fliken Automatic Export.
![](/img/posts/Zotero_export.png)
Olika editorer hanterar sedan referenser på olika sätt, men om man i TeXstudio anger var bib-filen finns kommer förslag på referenser upp från den när man börjar skriva ett kommando som används för att lägga in en referens, som t.ex. via `\parencite`.
...
```
Alternativt kan man använda R-paketet [`citr`](https://cran.r-project.org/web/packages/citr/index.html), som hämtar referenser direkt från Zotero via en add-in i RStudio. Den skapar sedan en lokal bib-fil. En nackdel är att den filen inte automatiskt uppdateras om man uppdaterar en referens i Zotero. Det är möjligt att det kommer i en senare uppdatering av paketet, men i dagsläget är det en brist med att välja det sättet.
